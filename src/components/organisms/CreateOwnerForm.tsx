...
import { createOwner } from "@/services/userService";
...
export default function CreateOwnerForm() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      // Invalidate and refetch data if needed, e.g., a list of owners
    ...
    onError: (error: Error) => {
        // The error from the mutation is now available here
    },
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (age < 18) {
        setValidationError("Owner must be at least 18 years old.");
        return;
    }
  ...
  return (
    <div className="mt-8">
        <h2 className="text-2xl font-bold mb-4">Create New Owner</h2>
        ...
        {mutation.isSuccess && <p className="text-green-500">Owner created successfully!</p>}
        ...
        <div>
            ...
            {mutation.isLoading ? 'Creating...' : 'Create Owner'}
            </button>
        </div>
        </form>
    </div>
